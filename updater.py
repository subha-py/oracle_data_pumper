...
parser = argparse.ArgumentParser(description='A program to update a db in oracle',
                                 usage='python3 updater.py --host 10.14.69.121 --db_name prodsb21 --user sys --password cohesity --batch_size 200000 --max_threads 128')
...
optional.add_argument('--max_threads', help='number of threads (default:128)', default=128, type=int)
...
def get_rows(connection, start_index=0, end_index=10):
    with connection.cursor() as cursor:
        cursor.execute(f"select * from (select q.*, rownum rn from (SELECT * FROM todoitem ORDER BY id) q ) where rn BETWEEN {start_index} AND {end_index}")
        for row in cursor:
            print(row)

def process_batch(connection, batch_size, batch_number, rows=None, number_of_batches=None, start_index=0):
    if not rows:
        rows = []
    toggle = 0
    ...
    with connection.cursor() as cursor:
        print('{}: updating batch number - :{}/{}'.format(datetime.datetime.now(), batch_number, number_of_batches))
        sql_update = "update todoitem set done = :1,randomnumber = :2,randomstring = :3  where id = :4"
        try:
            cursor.executemany(sql_update, rows, batcherrors=True)
            for error in cursor.getbatcherrors():
                print("{} :Error batch number - :{}/{}".format(datetime.datetime.now(), batch_number, number_of_batches), error.message, "at row offset", error.offset)
        except Exception as ex:
                print(ex)
    print('{}: committing batch number - :{}/{}'.format(datetime.datetime.now(), batch_number, number_of_batches))
    connection.commit()
...
def pump_updates(connection, batch_size, max_threads=128):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        for batch_number in range(1, number_of_batches + 1):
            arg = (connection, batch_size, batch_number, None, number_of_batches, batch_number*number_of_batches-1)
